...
public class getPosisioni {
    ...
    public String getXmlScheda(@PathParam(value = "idScheda") int idScheda) {
        ...
        sql = " SELECT latitudine, longitudine, dataOra FROM posizione WHERE idScheda =?";
        try (PreparedStatement statement = db.getConnection().prepareStatement(sql)) {
            statement.setInt(1, idScheda);
            ...
            try (ResultSet result = statement.executeQuery()) {
                ...
                while (result.next()) {
                    ...
                    ris.append("<latitudine>").append(result.getDouble(1)).append("</latitudine>");
                    ris.append("<longitudine>").append(result.getDouble(2)).append("</longitudine>");
