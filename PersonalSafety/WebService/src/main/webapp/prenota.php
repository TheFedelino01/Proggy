...
$elencoFasceOrarie="<tr><th>INIZIO ORA</th><th>FINE ORA</th><th>POSTI</th><th></th></tr>";

$filtroON=false;
$data=date("Y-m-d");//default giorno attuale
if( isset($_GET["filtroData"]) ){
	$data=$_GET["filtroData"];
	$filtroON=true;
}
...
$sql ="SELECT * FROM (ente join orari on ente.cod=orari.idEnte) where ente.cod='$idEnte' AND giorno='$dayOfWeek' order by giorno,oraInizio";
$result = $conn->query($sql);
...
while($row = $result->fetch_assoc()){
	...
	$oraFine=$row["oraFine"];
	$giorno=$row["giorno"];
	$posti = $row["postiDisponibili"];
	
	$elencoFasceOrarie.="<tr><td>$oraInizio</td> <td>$oraFine</td><td>$posti</td><td><input type='button' onClick='conferma(\"$oraInizio\",\"$oraFine\",\"$data\");' value='Seleziona' style='width:100%; height:100%;'></td></tr>";
}
...
<head>
	...
	<script>
		function conferma(inizio,fine,data){
			window.location='confermaPrenotazione.php?codEnte=<?php echo($_GET["codEnte"]); ?>&oraInizio='+inizio+'&oraFine='+fine+'&data='+data;
		}
		
		function filtraGiorno(){
			var dataSelezionata = document.getElementById("calendario").value;
			
			window.location='prenota.php?codEnte=<?php echo $idEnte; ?>&filtroData='+dataSelezionata;
		}
	</script>
...
<div class="container-lg p-responsive pb-6" style="padding-bottom: 0px!important;">
    ...
    <p class="lead-mktg text-gray text-center col-md-8 mx-auto mb-4">
		Stai per effettuare una Prenotazione:
		<br>
		Giorno: <input type="date" id="calendario" value="<?php echo date("Y-m-d");?>" min="<?php echo date("Y-m-d");?>" > <input type="button" value="OK" onClick="filtraGiorno()">
		<br>
		<br>
		
		Fasce orarie disponibili:
		<table>
			<?php echo $elencoFasceOrarie; ?>
		</table>
    </p>
...
<?php //Cosí se imposta il fitro sulla data, nel calendario mostro la data selezionata
if($filtroON==true){
	
	echo("<script>document.getElementById('calendario').value='$data'</script>");
}
?>
</html>